<template>
  <div class="main">
    <el-breadcrumb separator="/" class="breadcrumb" style="padding: 0 20px">
      ...
      <el-breadcrumb-item :to="{ path: '/complaint/list' }">投诉列表页</el-breadcrumb-item>
    ...
    <div class="content">
      <el-card v-loading="loading" shadow="hover" :body-style="{ padding: '12px', 'border-radius': '6px' }">
        ...
        <div class="file-wrap" v-if="files">
...
import { getComplaint } from '../../api/bid/complaint/complaint';
...
export default {
  ...
  mounted() {
    ...
    this.loading = true;
    getComplaint(this.id).then(response => {
      this.$nextTick(() => {
        this.data = response.data
        this.title = response.data.title
        this.content = response.data.content
        this.publishTime = response.data.createTime
        this.files = response.data.filesList
        this.status = this.selectDictLabel(this.statusOptions, response.data.status);
        if(response.data.status === '0'){
          this.statusColor['color'] = '#ffba00'
        }
        this.loading = false
      })
    })
