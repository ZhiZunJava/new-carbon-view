...
<script>
import { getPolicylawList } from '@/api/bid/policylaw/policylaw';
...
export default {
  ...
  methods: {
    getList() {
      this.loading = true
      getPolicylawList(this.queryParams).then(response => {
    ...
    toDetail(id) {
      this.$router.push({ path: '/policylaw/detail/', query: { id: id } })
    }
  }
}
</script>
